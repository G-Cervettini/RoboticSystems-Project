...
qvet_ini=deg2rad([0 45 -30 -10 -90 -30]);   % configurazione iniziale, spazio giunti [rad]
TCP_ini=rob6R.fkine(qvet_ini);              % sr TCP wrt 0 configurazione iniziale
deltaTCP=[-.3 .1 .2];                       % variazione di posizione TCP [m]
TCP_fin=transl(deltaTCP)*TCP_ini;           % sr TCP wrt 0 configurazione finale

stop_time=5;        % durata traiettoria e simulazione [s]
